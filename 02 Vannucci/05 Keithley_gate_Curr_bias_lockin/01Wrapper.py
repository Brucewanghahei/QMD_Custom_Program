...
class Collect_data(QThread):
    ...
    def input(self, ui, instruments, curves, curveWidgets, go_on, inputted_data, comments, array_sweep, inputted_string, input_string):
        ...
        self.current_bias_resistor = float(inputted_data[7])
        self.lockin_outputVoltage = float(inputted_data[8])
        ...
        while True:
            if self.go_on:
                ...
                if self.pause_collecting:
                    ...
                    DeviceCurr = float(self.lockin_outputVoltage)/float(self.current_bias_resistor)
