...
class MyForm(QMainWindow):
    ...
    def __init__(self, parent = None):
        ...
        self.connect(self.ui.pushButton_select_4, SIGNAL('clicked()'), lambda : self.Select("visa4", self.visa4, self.ui.comboBox_visa_4, self.ui.label_visa_4, [self.ui.pushButton_select_4, self.ui.pushButton_close_4], baud = 115200))
    ...
    def start(self):
        ...
        if not run_program:
            ...
            if str(self.ui.pushButton_Start.text()) == "Start":
                self.start_font("C")
                self.collect_data_thread.input(self.ui, instruments, curves, curveWidgets, go_on, inputted_data, comments, array_sweep, inputted_string)
            else:
                self.start_font("S")
...
class Collect_data(QThread):
    ...
    def input(self, ui, instruments, curves, curveWidgets, go_on, inputted_data, comments, array_sweep, inputted_string):
        self.ui = ui
        self.lockinCurr_visa = instruments[0]
        self.lockinVolt_visa = instruments[1]
        self.keithley_visa = instruments[2]
        self.magnet_visa = instruments[3]
        self.curves = curves
        ...
        self.lockin_curr_sens = float(inputted_data[6])
        self.lockin_volt_sens = float(inputted_data[7])
        self.curr_preamp_sens = float(inputted_data[8])
        self.volt_gain = float(inputted_data[9])
        self.curr_gain = float(inputted_data[10])
        self.user = str(inputted_data[11])
        self.file_name = str(inputted_data[12])
        self.timeStamp = str(inputted_data[13]).upper()
    ...
    def run(self):
        ...
        write_file.write("Time, Field, Gate Voltage, Gate Current, Device Voltage, Device Current, Device Conductane, Device Resistance\n")
        write_file.write("s, T, V, A, V, A, S, Ohms\n")
        ...
        while True:
            if self.go_on:
                ...
                Keithley().read_data_write(self.keithley_visa)
                gate_data = Keithley().read_data_read(self.keithley_visa)
                Agilent().read_data_write(self.lockinCurr_visa)
                Agilent().read_data_write(self.lockinVolt_visa)
                DeviceCurr = float(Agilent().read_data_read(self.lockinCurr_visa))
                DeviceVolt = float(Agilent().read_data_read(self.lockinVolt_visa))
                
                DeviceCurr = DeviceCurr*self.lockin_curr_sens*self.curr_preamp_sens/(10.0*self.curr_gain)
                DeviceVolt = DeviceVolt*self.lockin_volt_sens/(10.0*self.volt_gain)
                
                end_time = time.time()
                self.during = end_time - start_time
                self.t_plot.append(self.during)
                
                write_file.write(str(self.during) + "," + str(curr_field)+ "," + str(float(gate_data[0])) + "," + str(float(gate_data[1])) + "," + str(DeviceVolt)+ "," + str(DeviceCurr) + "," + str(DeviceCurr/DeviceVolt) + "," + str(DeviceVolt/DeviceCurr)+ "\n")
