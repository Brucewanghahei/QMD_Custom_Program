...
class MyForm(QMainWindow):
    ...
    def __init__(self, parent = None):
        ...
        #Bruces plot stuff may want to switch back to this at some pointt
        self.curve_1 = self.make_curveWidgets(self.ui.curvewidget_1, "b", "blue", titles = ["Current Ramp", "Step", "Current"])
        self.curve_1_update = self.make_curveWidgets(self.ui.curvewidget_1, "red", "red", titles = ["Current Ramp", "Step", "Current"])
        self.curve_2_b = self.make_curveWidgets(self.ui.curvewidget_2, "black", "black", titles = ["Plot 2", "X (x)", "Y (y)"])
        self.curve_2_r = self.make_curveWidgets(self.ui.curvewidget_2, "red", "red", titles = ["Plot 2", "X (x)", "Y (y)"])
        self.curve_3_b = self.make_curveWidgets(self.ui.curvewidget_3, "black", "black", titles = ["Voltage Out vs Current In", "Current (uA)", "Voltage (mV)"])
        self.curve_3_r = self.make_curveWidgets(self.ui.curvewidget_3, "red", "red", titles = ["Voltage Out vs Current In", "Current (uA)", "Voltage (mV)"])
        self.curve_4_b = self.make_curveWidgets(self.ui.curvewidget_4, "black", "black", titles = ["Differential Resistance vs Current In", "Current (uA)", "Resistance (Ohms)"])
        self.curve_4_r = self.make_curveWidgets(self.ui.curvewidget_4, "red", "red", titles = ["Differential Resistance vs Current In", "Current (uA)", "Resistance (Ohms)"])
        ...
        self.Curve4 = self.make_curveWidgets(self.ui.curvewidget_4, "blue", "black", titles = ["Differential Resistance vs Current In", "Current (uA)", "Resistance (Ohms)"])
        ...
        self.input_string.append("19 Sensitivity of Lockin (uV): 500\n")
        self.input_string.append("20 Lockin Resistor (MOhms): 1\n")
        self.input_string.append("21 Output Voltage(mV): 50\n")
        self.input_string.append("22 Rf Frequemcy: 2 Ghz\n")
        self.input_string.append("23 Rf Power: 0 dB\n")
        self.input_string.append("24 Temperature: 100 mK\n")
    ...
    def start(self):
        ...
        #.read().replace("\n", "")
        #diff_resist = (lockin_volt*lockin_sense*lockin_resist)/(10*pre_amp*output_volt)
        #print string_
        ...
        self.instruments = []
        curves = [self.Curve1, self.Curve2, self.Curve3, self.Curve4, self.curve_1, self.curve_1_update]         #refrences corresponding make.curvewidgets 
    ...
    def Check_ready(self, count):
        if count == 1:
            if self.ui.label_visa_1.text() != '' and self.ui.label_visa_2.text() != '':
                self.instruments = [self.visa1, self.visa2, self.visa4]
                return True
...
class Collect_data(QThread):
    ...
    def Reading(self, device, input_volt, num, Round):
        ...
        if device == "Yokogawa1":
            ...
            lockin_sense = float(self.Data[2][16])/(10**6)
            lockin_resist = float(self.Data[2][17])*(10**6)
            output_volt = float(self.Data[2][18])/(10**3)
            #agilent collect
            ...
            lockin_volt = float(self.agilent2.read().replace("\n", ""))
            diff_resist = (lockin_volt*lockin_sense*lockin_resist)/(10*pre_amp*output_volt)
            #print string_
            list_ = numpy.fromstring(string_, dtype = numpy.float32, count=int(self.Data[2][3]), sep=',')
            volt = numpy.mean(list_)/ pre_amp
        ...
        if self.ui.checkBox_dsave.isChecked():
            ...
            if is_last:
                ...
                if num == 0:
                    ...
                    #temp = str(self.ui.textEdit.toPlainText())
                    temp = []
                    ...
                    comments.append(temp)
                    temp = []
                    temp.append(self.input_data[j + 16])
                    comments.append(temp)
                    temp = []
                    temp.append(self.input_data[j + 17])
                    comments.append(temp)
                    temp = []
                    temp.append(self.input_data[j + 18])
                    comments.append(temp)
                    temp = []
                    temp.append(self.input_data[j + 19])
                    comments.append(temp)
                    temp = []
                    temp.append(self.input_data[j + 20])
                    comments.append(temp)
                    temp = []
                    temp.append(self.input_data[j + 21])
                    comments.append(temp)
                    """
